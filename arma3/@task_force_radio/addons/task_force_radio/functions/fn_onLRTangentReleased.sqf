...
if ((TF_tangent_lr_pressed) and {alive player}) then {
	...
	_request = format["TANGENT_LR	RELEASED	%1%2	%3", call TFAR_fnc_currentLRFrequency, (call TFAR_fnc_activeLrRadio) call TFAR_fnc_getLrRadioCode, [_radio select 0, "tf_range"] call TFAR_fnc_getLrRadioProperty];
