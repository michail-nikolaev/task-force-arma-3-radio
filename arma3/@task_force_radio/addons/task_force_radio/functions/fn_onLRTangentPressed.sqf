...
if (!(TF_tangent_lr_pressed) and {alive player} and {call TFAR_fnc_haveLRRadio}) then {
	if ([player, player call TFAR_fnc_vehicleIsIsolatedAndInside] call TFAR_fnc_canUseLRRadio) then {
		...
		_request = format["TANGENT_LR	PRESSED	%1%2	%3", call TFAR_fnc_currentLRFrequency, _radio call TFAR_fnc_getLrRadioCode, [_radio select 0, "tf_range"] call TFAR_fnc_getLrRadioProperty];
