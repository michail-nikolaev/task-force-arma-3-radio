...
<p align="center">
<sup><strong>Радио плагин <a href="http://www.teamspeak.com/">Тимспика</a> для Arma 3. TFAR уже одни из финалистов конкурса <a href="http://makearmanotwar.com/entry/pMP8c7vSS4#.VA1em_nV9UD">Nake Arma Not War</a>.</strong></sup>
</p>
...
* Скопируйте содержимое папки `TeamSpeak 3 Client` в корневую папку Тимспика.
* Скопируйте содержимое папки `Arma 3` в папку с игрой — `...\SteamApps\common\Arma 3`.

> Каждый выпуск [TFAR](http://radio.task-force.ru/) содержит последнюю версию [СBA](http://www.armaholic.com/page.php?id=18767) (Community Base Addons) как часть архива. Если у вас уже установлен CBA, но не уверены в его версии, то лучше дать Windows заменить существующую папку.

###Настройка
* Убедитесь, что Тимспик не использует клавишу <kbd>Caps Lock</kbd> для разговора.
* В настройх игры, чтобы не двоиться, отключите разговор по внутренней связи (VON), или поменяйте его использование с <kbd>Caps Lock</kbd> на любую другую клавишу.
* В Тимспике, откройте список плагинов – `Settings> Plugins`.
...
* бедитесь, что в Тимпсике не отключена громкость оповещений – `Options> Payback> Sound Pack Volume` – установите в положительное значение.
* Запустите игру со включенными `@CBA_A3` и `@task_force_radio`. аддонами. Вы можете это сделать прописав их имена в ярлык игры, сразу после EXE файла — `…\arma3.exe -mod=@CBA_A3;@task_force_radio`. Тем не менее, собственное меню игры является более предпочтительным для этого местом (`Settings> Expansions`).
* В Тимспике, присоединяйтесь на тот же канал что и другие игроки с рацией. Играя в серьёзном режиме, на сервере будет канал под названием `TaskForceRadio`, куда по началу общей миссии все участвующие игроки будут переброшены автоматически.
