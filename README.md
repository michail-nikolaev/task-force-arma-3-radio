...
5. Запустите игру со включенными `@CBA_A3` и `@task_force_radio` аддонами. Это можно сделать прописав их названия в ярлык игры, либо сразу после `exe` файла — `…\arma3.exe -mod=@CBA_A3;@task_force_radio`. Тем не менее, собственное меню игры является более предпочтительным методом настройки (`Settings> Expansions`).
