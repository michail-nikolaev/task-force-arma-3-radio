...
* Скачайте [архив с рацией 0.5.0 alpha](https://github.com/michail-nikolaev/task-force-arma-3-radio/raw/master/releases/0.5.0%20alpha.zip) и распакуйте его.
...
* Если у вас 32 битная версия TeamSpeak, скопируйте содержимое папки `TeamSpeak 3 Client\x32` в корневую папку TeamSpeak. Если 64 битная версия, то содержимое папки`TeamSpeak 3 Client\x64`.

> Чтобы узнать вашу версию TS, зайдите в корневую папку TeamSpeak. В ней вы найдете файл `ts3client_win64.exe` или `ts3client_win32.exe`.
...
| `ALT` + `P` | Открыть интерфейс рации дальней связи (рация должна быть в слоте инвентаря). Для того, чтобы ввести частоту нажмите `MENU CLR`, введите значение и нажмите `FREQ`. |
