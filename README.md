...
| `ALT`&nbsp;+&nbsp;`P` | Открыть интерфейс рации дальней связи (рация дальней связи должна быть одета на спину, либо вы должны быть в технике за водителя, стрелка, командира или помощника пилота). Если доступно несколько рации - вам будет предложено выбрать. Также одну из них можно установить как активную. |
...
#####Выдача раций
* По умолчанию рация дальней связи выдается лидерам отрядов. Если у и игрока надет рюкзак - он его автоматически положит на землю.
...
#####Водолазы
* Вы не можете говорить голосом находясь под водой (даже в водолазном костюме). Однако на близком расстояние ваш собеседник сможет расслышать что-либо очень невнятное (исключение - если вы под водой в изолированной технике).
...
tf_east_radio_code = "_opfor"; tf_guer_radio_code = "_independent"; `. Для того, чтобы две фракции могли связываться по радио них должен быть идентичный код шифрования (вам потребуется поменять значения).
