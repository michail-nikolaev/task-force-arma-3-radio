...
1. В настройх игры, чтобы не двоиться, отключите разговор по внутренней связи (VON), или поменяйте его использование с <kbd>Caps Lock</kbd> на любую другую клавишу.
1. В Тимспике, откройте список плагинов – `Settings> Plugins`.
   1. Включите `Task Force Arma 3 Radio`.
   1. Отключите `ACRE` и другие подобные аддоны раций, если они у вас есть, чтобы избежать возможных конфликтов.
   1. Нажмите кнопку `Reload All` (внизу, слева) для перезагрузки всех текущих плагинов.
1. Убедитесь, что в Тимпсике не отключена громкость оповещений – `Options> Payback> Sound Pack Volume` – установите в положительное значение.
1. Запустите игру со включенными `@CBA_A3` и `@task_force_radio` аддонами. Это можно сделать прописав их названия в ярлык игры, либо сразу после `exe` файла — `…\arma3.exe -mod=@CBA_A3;@task_force_radio`. Тем не менее, собственное меню игры (`Settings> Expansions`) является более предпочтительным методом настройки.
1. В Тимспике, присоединяйтесь на тот же канал что и другие игроки с рацией. Играя в серьёзном режиме, на сервере будет канал под названием `TaskForceRadio`, куда по началу общей миссии все участвующие игроки будут переброшены автоматически.
