...
5. Запустите игру со включенными `@CBA_A3` и `@task_force_radio`. аддонами. Вы можете это сделать прописав их имена в ярлык игры, сразу после EXE файла — `…\arma3.exe -mod=@CBA_A3;@task_force_radio`. Тем не менее, собственное меню игры является более предпочтительным для этого местом (`Settings> Expansions`).
...
* [marc_book] (https://github.com/MarcBook) за текстуры для [BWMOD](http://bwmod.de/).
