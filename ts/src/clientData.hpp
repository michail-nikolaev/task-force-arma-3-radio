...
public:
	...
	std::string getNickname() const { LockGuard_shared lock(&m_lock); return nickname; }
	...
	dataType::Direction3D getViewDirection() const { LockGuard_shared lock(&m_lock); return viewDirection; }
	void setViewDirection(const Direction3D& val) { LockGuard_exclusive lock(&m_lock); viewDirection = val; }
	std::chrono::system_clock::time_point getLastPositionUpdateTime() const { LockGuard_shared lock(&m_lock); return lastPositionUpdateTime; }
	...
	std::string getCurrentTransmittingFrequency() const { LockGuard_shared lock(&m_lock); return currentTransmittingFrequency; }
	...
	std::string getCurrentTransmittingSubtype() const { LockGuard_shared lock(&m_lock); return currentTransmittingSubtype; }
	void setCurrentTransmittingSubtype(const std::string& val) { LockGuard_exclusive lock(&m_lock); currentTransmittingSubtype = val; }
	std::string getVehicleId() const { LockGuard_shared lock(&m_lock); return vehicleId; }
