...
std::vector<LISTED_INFO> clientData::isOverRadio(std::shared_ptr<clientData>& myData, bool ignoreSwTangent, bool ignoreLrTangent, bool ignoreDdTangent) {
    ...
    if (
        ...
        auto speakersRange = TFAR::getInstance().m_gameData.speakers.equal_range(getCurrentTransmittingFrequency());

        for (std::multimap<std::string, SPEAKER_DATA>::iterator itr = speakersRange.first; itr != speakersRange.second; ++itr) {
            SPEAKER_DATA& speaker = itr->second;
            auto speakerOwner = speaker.client.lock();
            if (!speakerOwner) continue;
            ...
            if (speakerOwner->clientId == clientId) continue;
            result.emplace_back(
