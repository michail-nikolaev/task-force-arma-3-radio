	...
	if (call TFAR_fnc_isAbleToUseRadio) then {
		...
		if ([player, player call TFAR_fnc_vehicleIsIsolatedAndInside, player call TFAR_fnc_eyeDepth] call TFAR_fnc_canUseLRRadio) then {
			...
			if ((_radio call TFAR_fnc_getAdditionalLrChannel) > -1) then {
				_hintText = format[localize "STR_additional_transmit",format ["%1<img size='1.5' image='%2'/>",[_radio select 0, "displayName"] call TFAR_fnc_getLrRadioProperty, getText(configFile >> "CfgVehicles"  >> typeof (_radio select 0) >> "picture")], (_radio call TFAR_fnc_getAdditionalLrChannel) + 1, _freq];
				[parseText (_hintText), -1] call TFAR_fnc_showHint;
				_request = format["TANGENT_LR	PRESSED	%1%2	%3	%4", _freq, _radio call TFAR_fnc_getLrRadioCode, ([_radio select 0, "tf_range"] call TFAR_fnc_getLrRadioProperty)  * (call TFAR_fnc_getTransmittingDistanceMultiplicator), [_radio select 0, "tf_subtype"] call TFAR_fnc_getLrRadioProperty];
