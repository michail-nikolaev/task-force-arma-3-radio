...
_menu = [];
if (count (currentUnit call TFAR_fnc_lrRadiosList) > 1) then {
	_menuDef = ["main", localize "STR_select_lr_radio", "buttonList", "", false];
	_positions = [];
	_pos = 0;
	{
		_command = format["TF_lr_dialog_radio = (currentUnit call TFAR_fnc_lrRadiosList) select %1;call TFAR_fnc_onLrDialogOpen;", _pos];
		_submenu = "";
		...
		if (((_x select 0) != (_active_radio select 0)) or ((_x select 1) != (_active_radio select 1))) then {
			_command = format["TF_lr_dialog_radio = (currentUnit call TFAR_fnc_lrRadiosList) select %1;", _pos];
