#define ADD_PHONE_ACTIONS(parent, child) \
class child : parent { \
    class ACE_Actions { \
...
#define ADD_WIRELESS_ACTIONS(parent, child) \
class child : parent { \
    class ACE_Actions { \
        class ACE_MainActions { \
            ...
            class TFAR_External_Intercom_Wireless_Connect : TFAR_External_Intercom_Wireless_Base { \
                ...
                condition = "([(typeOf _target), 'TFAR_hasIntercom', 0] call TFAR_fnc_getVehicleConfigProperty) > 0 && ((_target getVariable ['TFAR_ExternalIntercomSpeakers', [objNull, []]] select 1) find _player) < 0 && (headgear _player) in TFAR_externalIntercomWirelessHeadgear"; \
