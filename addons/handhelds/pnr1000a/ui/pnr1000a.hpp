...
class pnr1000a_radio_dialog
{
    ...
    onLoad = QUOTE(if (sunOrMoon < 0.2) then {((_this select 0) displayCtrl TF_IDD_BACKGROUND) ctrlSetText 'PATHTOF(pnr1000a\ui\pnr1000a_n.paa)';((_this select 0) displayCtrl IDC_MICRODAGR_BACKGROUND) ctrlSetText 'PATHTOF(microdagr\ui\microdagr_n.paa)';};_this call TFAR_fnc_updateProgrammatorDialog;);
    controls[]=
    {
    	background,
    	prev_channel,
    ...
    class clear: HiddenButton
    {
    	...
    	font = "TFAR_font_dots";
    	shadow = 2;
    	action = QUOTE(ctrlSetText [ARR_2(IDC_MICRODAGR_EDIT, '')];ctrlSetFocus ((findDisplay IDC_PNR1000A_RADIO_DIALOG) displayCtrl IDC_MICRODAGR_EDIT););
    };
