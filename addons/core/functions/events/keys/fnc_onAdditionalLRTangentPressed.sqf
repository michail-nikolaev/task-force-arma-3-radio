    ...
    if (call TFAR_fnc_isAbleToUseRadio) then {
        if ([TFAR_currentUnit, TFAR_currentUnit call TFAR_fnc_vehicleIsIsolatedAndInside, TFAR_currentUnit call TFAR_fnc_eyeDepth] call TFAR_fnc_canUseLRRadio) then {
            ...
            if ((_radio call TFAR_fnc_getAdditionalLrChannel) > -1) then {
                ...
                format["TANGENT_LR	PRESSED	%1%2	%3	%4	%5", _freq, _radio call TFAR_fnc_getLrRadioCode, ([_radio select 0, "tf_range"] call TFAR_fnc_getLrRadioProperty)  * (call TFAR_fnc_getTransmittingDistanceMultiplicator), [_radio select 0, "tf_subtype"] call TFAR_fnc_getLrRadioProperty, typeOf (_radio select 0)],
                -1
