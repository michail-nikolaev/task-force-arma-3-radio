...
TRACE_2("input",_valueString,parseSimpleArray _valueString);
private _parsedValue = (parseSimpleArray _valueString) apply {
    If (IS_STRING(_x)) then {parseNumber _x} else {_x};
};
TRACE_3("input",_parsedValue,_minFreq,_maxFreq);
...
_parsedValue = _parsedValue apply {_x toFixed _roundPower};
TRACE_2("Channelamount",count _parsedValue,_maxChannels);
